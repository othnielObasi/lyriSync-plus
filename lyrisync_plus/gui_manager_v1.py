...
class LyriSyncGUI:
    ...
    def _build_status_tab(self):
        ...
        entry = ttk.Entry(test, textvariable=self._lyrics_var, width=52)
        entry.grid(row=0, column=1, sticky="ew", padx=6, pady=5)

        ttk.Button(test, text="Send to vMix", command=self._send_test_lyrics, bootstyle=SUCCESS).grid(
        ...
        ttk.Button(actions, text="Show Lyrics", command=lambda: self._trigger_action("show_lyrics"), bootstyle=INFO).pack(
            side="left", padx=5
        )
    ...
    def _send_test_lyrics(self):
        if not callable(self.action_callback):
            return
        txt = (self._lyrics_var.get() or "").strip()
        if not txt:
            return
        try:
...
class SettingsDialog:
    def __init__(self, parent, config: Dict[str, Any], discoverer: AsyncVmixDiscoverer, on_apply: Callable[[Dict[str, Any]], None]):
        ...
        self.wrap_var = tk.StringVar(value=str(s.get("max_chars_per_line", 48)))
    ...
    def _save_settings(self):
        try:
            new_settings = {
                ...
                "max_chars_per_line": max(10, int(self.wrap_var.get().strip() or "48")),
